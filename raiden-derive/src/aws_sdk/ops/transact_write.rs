        ...
        impl ::raiden::TransactWriteUpdateBuilder for #update_builder {
            fn build(mut self) -> ::raiden::Update {
                ...
                if self.builder.get_expression_attribute_names().is_none() {
                    ...
                    let v = self.builder.get_expression_attribute_names().clone().unwrap();
                    self.builder = self.builder
                        .set_expression_attribute_names(Some(::raiden::merge_map(v, attr_names)));
                }
                ...
                if self.builder.get_expression_attribute_values().is_none() {
                    ...
                    let v = self.builder.get_expression_attribute_values().clone().unwrap();
                    self.builder = self.builder
                        .set_expression_attribute_values(Some(::raiden::merge_map(v, attr_values)));
                }

                self.builder = self.builder.update_expression(update_expression);
                self.builder = self.builder
                    .table_name(format!("{}{}{}", self.table_prefix, self.table_name, self.table_suffix));
