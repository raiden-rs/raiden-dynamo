...
///
/// When a source model uses `#[raiden(omit_gsi = "...")]` or
/// `#[raiden(omit_lsi = "...")]`, `#[derive(Raiden)]` also generates a default
/// projection item for that index.
...
/// Marks a projected item type as belonging to a specific source model and secondary index.
///
/// This trait is generated by `#[derive(RaidenIndex)]` and by the automatic
/// `omit_gsi`/`omit_lsi`-based projection generation in `#[derive(Raiden)]`.
///
/// Typed index builders use this trait to switch both the projection expression
/// and the output item type.
pub trait RaidenIndexItem<Source>: RaidenItem {
    /// The index name associated with this projection type.
    ///
    /// Retained as the required constant for compatibility with existing implementations.
    const GSI_NAME: &'static str;

    /// The DynamoDB secondary index name associated with this projection type.
    const INDEX_NAME: &'static str = Self::GSI_NAME;
}
